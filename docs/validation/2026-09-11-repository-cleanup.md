...
- Test-tooling PR #11 and Actions PR #17 were incorporated by merge commits in #19. The MSTest upgrade additionally replaces the obsolete `DataTestMethod` attribute with `TestMethod`, preserving all five migration data rows. No test is removed or suppressed. Coverlet 10.0.1 restored successfully but failed real coverage collection: its package contains only net8/net9/net10 build imports, none selected by this net48 project. The collector is updated from 6.0.0 to compatible 6.0.4 instead. Follow-up package inspection confirmed the same missing net48 import in 8.0.0, 8.0.1 and 10.0.0; Dependabot excludes those four exact releases, while future versions remain reviewable.
...
## Follow-up dependency refresh

PR #19 merged as `522353d1dcb41c4705513b257f49b79087c326d5`. Its final Windows run `34545469853`, three-browser run `34545469909`, post-merge build `34545902646` and Pages deployment `34545902644` all passed. GitHub's actual README rendering was verified with the app icon, both current screenshots and the historical-demo toggle.

The configuration change triggered an immediate Dependabot scan, which opened #20 for Microsoft.NET.Test.Sdk 18.10.0 together with Coverlet 10.0.0. The SDK update was retained and independently validated with a warning-free Release build and 237/237 tests with real coverage. Only the incompatible collector change was removed. The exact-version exclusions above prevent cycling through older incompatible collector releases. Package archive import listings and the SDK test results are retained in the local recovery directory.
